...
const SourceEditor = (props: { language: string }) => {
  ...
  const updateSavedPrompts = async () => {
    ...
    // console.log(prompts)
    setSavedPrompts(prompts.filter((d) => d.type === "source"));
  ...
  async function updateList() {
    getSavedPrompts().then((sp) => {
      // console.log(sp)
      setSavedPrompts(sp.filter((d) => d.type === "source"));
    });
  }
  ...
  const handleAdd = () => {
    ...
    setPrompt({ name: "", text: "", type: "source" });
    setDeleteBtnText("delete");
  ...
  return (
    <div className="wcg-rounded-box wcg-mt-10 wcg-flex wcg-h-[12rem] wcg-w-4/5 wcg-flex-row wcg-gap-4 wcg-border wcg-py-4">
      <div className="wcg-w-1/3">{PromptList}</div>
